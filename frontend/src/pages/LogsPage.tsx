...
const logColumns: Column<ActivityEvent>[] = [
  {
    key: 'time',
    header: 'Time',
    cellClassName: 'text-gray-400 text-xs whitespace-nowrap font-mono',
    render: ev => formatTime(ev.created_at),
  },
  {
    key: 'severity',
    header: 'Severity',
  ...
  },
  {
    key: 'kind',
    header: 'Kind',
    cellClassName: 'text-xs text-gray-600 font-mono whitespace-nowrap',
    render: ev => ev.kind,
  },
  {
    key: 'message',
...
export default function LogsPage() {
  ...
  const hasMore = (data?.events?.length ?? 0) === FETCH_BATCH_SIZE
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
        {/* Filter bar — full-bleed, preserved from previous layout */}
        ...
        {error ? (
          <p className="px-6 py-4 text-sm text-red-500">Failed to load events</p>
        ) : visibleEvents.length === 0 ? (
          <p className="px-6 py-8 text-sm text-gray-400 text-center">
            {isLoading ? 'Loading…' : 'No events match the current filters.'}
          </p>
        ) : (
          <div className="px-4 py-3">
            <PaginatedTable
              ...
              rowKey={ev => ev.id}
              defaultPageSize={25}
              headerRowClassName="border-b border-gray-100 bg-gray-50/50"
            />
          </div>
        )}

...
func buildRecoveryScript(clientToml string) string {
	...
	b.WriteString("$SUDO rm -f /etc/rathole/.client.toml.swp\n")
	b.WriteString("$SUDO tee /etc/rathole/client.toml > /dev/null <<'GOPHER_CONFIG_EOF'\n")
	b.WriteString(clientToml)
	if !strings.HasSuffix(clientToml, "\n") {
		b.WriteString("\n")
	}
	b.WriteString("GOPHER_CONFIG_EOF\n")
	b.WriteString("if id -u gopher >/dev/null 2>&1; then $SUDO chown gopher:gopher /etc/rathole/client.toml; fi\n")
	b.WriteString("$SUDO chmod 0644 /etc/rathole/client.toml\n")
